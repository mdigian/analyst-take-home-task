...
SELECT code,
  ...
  FROM medications           --select *  FROM medications 
  WHERE description IN ('Hydromorphone 325 MG', 'Fentanyl 100 MCG','Oxycodone-acetaminophen 100ML','Amlodipine 5 MG / Fentanyl 100 MCG / Olmesartan medoxomil 20 MG') 
...
WITH patient_med_list AS ( --312028 encounters with active med rx.   Convert CTE to dedicated view to reuse
                           ...
                           SELECT e.patient                                                         AS patient ,
                                  ...
                                  MAX(CASE WHEN m.code IN (316049,429503,406022
                                                          ,999969                ) THEN 1 ELSE 0 END) AS CURRENT_OPIOID_IND
                             FROM encounters  e
 ...
 LEFT JOIN patient_med_list pml ON enc.patient = pml.patient 
     ...
     WHERE REASONDESCRIPTION LIKE '%Drug overdose%'
